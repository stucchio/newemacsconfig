...
(defun mu4e-windows-compatify-filenames (filename)
  "On windows, we must use ! instead of :"
  (if mu4e-windows-compatible-filenames
      (replace-regexp-in-string ":" "!" filename)
    filename
    ))

(defun mu4e-expand-file-name (filename)
  "On windows, need to modify the root"
  (if mu4e-windows-compatible-filenames
      (replace-regexp-in-string "/mnt/c" "c:" (expand-file-name filename))
